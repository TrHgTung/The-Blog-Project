...
2.  Chạy script:
    ...
    ./dev-start.ps1
    ```
    *Script này sẽ tự động: Khởi động Docker Infra -> Build Shared Library -> Mở các cửa sổ terminal riêng biệt cho từng Service & Front-end.*
...
- Do khả năng về tài chính có hạn để triển khai thực tế cho dự án này nên tôi khuyến nghị sẽ chọn cách triển khai dưới môi trường test. Tuy nhiên, một dự án thực tế tương tự (nhưng kiến trúc sẽ tinh gọn hơn để phù hợp với việc triển khai thực tế) sẽ có [tại đây](https://github.com/TrHgTung/the-social-project).